  ...
  produce(state, draft => {
    ...
    switch (action.type) {
      ...
      case DELETE_HERO:
        ...
        if (index != -1) {
          if (action.options.killThisEntity) {
            draft[index].curr_hp = 0;
            draft[index].curr_mp = 0;
            draft[index].curr_fp = 0;
            draft[index].poisoning = false;
            draft[index].burning = false;
            draft[index].bleeding = false;
          } else if (action.options.deleteThisEntity) {
